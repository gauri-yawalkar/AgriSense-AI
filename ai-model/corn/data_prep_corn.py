...
def move_corn_folders(base_dir):
    # Source where the original corn folders currently are
    source_dir = os.path.join(base_dir, "Crop-disease-02")
    dest_dir = os.path.join(base_dir, "corn")
    
    print(f"Moving corn folders from {source_dir} to {dest_dir}...")
    os.makedirs(dest_dir, exist_ok=True)
        
    if os.path.exists(source_dir):
        for folder_name in os.listdir(source_dir):
            if folder_name.lower().startswith("corn"):
                src_path = os.path.join(source_dir, folder_name)
                dst_path = os.path.join(dest_dir, folder_name)
                # Move if it hasn't been moved already
...
def prepare_corn_dataset(base_dir):
    input_dir = os.path.join(base_dir, "corn")
    output_dir = os.path.join(base_dir, "corn_split")
...
if __name__ == "__main__":
    # Absolute path to the raw_datasets folder we created earlier
    raw_datasets_dir = r"c:\T4T\leaf disease\indian-leaf-disease\raw_datasets"
    
    # 1. First, move the corn folders out of Crop-disease-02 into the new corn directory
    move_corn_folders(raw_datasets_dir)
    
    # 2. Then, split them into a train/val structure ready for PyTorch classification
    prepare_corn_dataset(raw_datasets_dir)
