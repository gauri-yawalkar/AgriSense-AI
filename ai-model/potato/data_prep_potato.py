...
def move_potato_folders(base_dir):
    # Source where the original potato folders currently are
    source_dir = os.path.join(base_dir, "Crop-disease-02")
    dest_dir = os.path.join(base_dir, "potato")
    
    print(f"Moving potato folders from {source_dir} to {dest_dir}...")
    os.makedirs(dest_dir, exist_ok=True)
        
    if os.path.exists(source_dir):
        for folder_name in os.listdir(source_dir):
            if folder_name.lower().startswith("potato"):
...
def prepare_potato_dataset(base_dir):
    input_dir = os.path.join(base_dir, "potato")
    output_dir = os.path.join(base_dir, "potato_split")
...
if __name__ == "__main__":
    # Absolute path to the raw_datasets folder
    raw_datasets_dir = r"c:\T4T\leaf disease\indian-leaf-disease\raw_datasets"
    
    # 1. Move potato folders
    move_potato_folders(raw_datasets_dir)
    
    # 2. Split them into train/val
    prepare_potato_dataset(raw_datasets_dir)
