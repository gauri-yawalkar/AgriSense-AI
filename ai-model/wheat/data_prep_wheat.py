...
def move_wheat_folders(base_dir):
    source_dir = os.path.join(base_dir, "Crop-disease-02")
    dest_dir = os.path.join(base_dir, "wheat")
    
    print(f"Moving wheat folders from {source_dir} to {dest_dir}...")
    os.makedirs(dest_dir, exist_ok=True)
        
    if os.path.exists(source_dir):
        for folder_name in os.listdir(source_dir):
            if folder_name.lower().startswith("wheat"):
...
def prepare_wheat_dataset(base_dir):
    input_dir = os.path.join(base_dir, "wheat")
    output_dir = os.path.join(base_dir, "wheat_split")
...
if __name__ == "__main__":
    ...
    move_wheat_folders(raw_datasets_dir)
    prepare_wheat_dataset(raw_datasets_dir)
